...
$(document).ready(function () {
    ...
    $(document).on("click", "#btnhitung", () => {
        getDataToCalculate((data) => {
            ...
            const resultFuzzy = calculateFuzzy(grouppedData)
            renderTable(resultCF, resultFuzzy)
        })
    })
})
...
function calculateFuzzyPerPenyakit(data) {
    ...
    return defuzzifikasi(fuzzyfikasi)
}
...
function defuzzifikasi(data) {
    ...
    const sumByGroup = defuzzifikasiPenyakit.reduce((acc, curr) => {
        acc[curr['nilaiFuzzy']] += 1
        return acc
    }, hierarchy.reduce((obj, item) => {
        obj[item] = 0
        return obj
    }, {}))

    const finalVal = Object.entries(sumByGroup).reduce((acc, curr) => acc = (curr[1] > acc[1]) ? curr : acc)[0]
    return finalVal
}
